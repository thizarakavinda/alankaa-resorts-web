    ...
    <section className="bg-void py-[120px] pl-6 md:pl-[80px] overflow-hidden">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-16 pr-6 md:pr-[80px] relative z-10 w-full">
        <div className="max-w-[480px]">
          <span className="font-jost text-[12px] text-gold uppercase tracking-[0.4em] block mb-4" data-aos="fade-right">What We Offer</span>
      ...
      <div className="flex overflow-x-auto gap-6 pb-8 snap-x snap-mandatory cursor-grab active:cursor-grabbing w-full scrollbar-none" style={{ scrollbarWidth: 'none' }}>
        {shootCards.map((card, i) => (
          ...
          >
            ...
            <div className="absolute top-6 left-6 font-jost text-[12px] text-void bg-gold px-[14px] py-[6px] tracking-[0.15em] uppercase z-10 shadow-lg">
              {card.badge}
            </div>

            <div className="absolute bottom-0 left-0 right-0 p-10 z-10">
              <span className="gold-label-on-image font-jost text-[12px] text-gold uppercase tracking-widest block mb-1">{card.category}</span>
              <h3 className="card-heading-on-image font-cormorant text-[38px] text-ivory leading-none mb-3">{card.name}</h3>
              <p className="card-text-on-image font-dmSans text-[15px] text-white/60 font-light leading-relaxed mb-4">{card.desc}</p>
              
              <div className="w-[40px] h-[1px] bg-gold mb-4 opacity-100" />
      ...
      <div className="flex justify-center mt-8">
        <span className="font-jost text-[12px] text-gold tracking-[0.3em] uppercase animate-pulse">
          ← Drag to Explore →
        </span>
      </div>
    </section>
